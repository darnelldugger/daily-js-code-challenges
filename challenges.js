...
function addList(...theArgs){

  return theArgs.reduce((income, outcome) => income += outcome, 0)
  }
...
function computeRemainder(diven, divis){
  if(divis === 0){
    return Infinity
  } else {
    return diven % divis
...
function reverseUpcaseString(revStr) {
  let splitStr = revStr.split("")
  let revArr = splitStr.reverse()
  let joinStr = revArr.join("")
  return reverseUpcaseString = joinStr.toUpperCase()
}
...
function charCount(str){
  ...
  let count = charArray.reduce(function(allChar, char){
    if(char in allChar){
      allChar[char]++
    } else {
      allChar[char] = 1
...
//   let re = /[W_]/g
//   let regStr = strg.replace(re, '')
//   let strgPalin = regStr.split('').reverse().join('')
//   if(strgPalin.length === 0 || strgPalin.length === 1){
//     return true
//   }
//   return strgPalin === regStr
// }
